...
namespace ECE141 {
  ...
  RowCollection& RowCollection::reorder(const PropertyList &anOrder, Entity &anEntity) {
    try {
      Property theProperty = anOrder[0];
      anEntity.getAttribute(theProperty.name);
      if (theProperty.desc) {
        sort(rows.begin(), rows.end(), [theProperty](Row *aLHS, Row *aRHS) {
        return aLHS->columns[theProperty.name]
                  < aRHS->columns[theProperty.name];
        });
      }
      else {
        sort(rows.begin(), rows.end(), [theProperty](Row *aLHS, Row *aRHS) {
