...
namespace ECE141 {
  ...
  StatusResult createRow(InsertStatement &aStatement, std::map<std::string,std::string> &aKVList, Entity &anEntity) {
    ...
    for(auto &theAttr : theAttributes) {
      ...
      Value theValue("");
      if(aKVList.count(theName)) { //If this attribute is already in the KVList given by the user
        theValue=aKVList[theName]; //use the value we were given...
      }
      theValue.become(theAttr.getType()); //Converts the value to string form
                                          //also to default value if theValue has an empty value
  ...
  StatusResult UpdateStatement::run(std::ostream &aStream)  const{
    return interpreter.updateRows(const_cast<KeyValues&>(keyValues), name, filters);
  }
}
