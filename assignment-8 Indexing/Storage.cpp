...
namespace ECE141 {
  ...
  bool BlockCache::contains(uint32_t aBlockNum) {
    try {
	  cacheMap.at(aBlockNum);
      return true;
    }
    catch(...) {
      return false;
  ...
  Block& BlockCache::get(uint32_t aBlockNum) {
    return cacheMap.at(aBlockNum);
  } //retrieve from cache (may throw exception)

  void BlockCache::put(uint32_t aBlockNum, const Block &aBlock) {
    if (contains(aBlockNum)) {
      //so already in list
      cacheList.remove(aBlockNum);//check this
      cacheList.push_front(aBlockNum);
      if (cacheMap[aBlockNum].data != aBlock.data) { //might need to update data
        cacheMap[aBlockNum] = aBlock;
      }
    }
    else {// not in cacheList
      if (cacheList.size() >= size) {
    		uint32_t toErase = cacheList.back();
    		cacheMap.erase(toErase);
    		cacheList.pop_back();
      }
      cacheList.push_front(aBlockNum);
	    cacheMap.insert({aBlockNum, aBlock});
    }
  }
  ...
  StatusResult  BlockIO::readBlock(uint32_t aBlockNumber, Block &aBlock) {
    ...
    if(stream.read ((char*)&aBlock, sizeof(Block))) {
  ...
  StatusResult  CachedBlockIO::readBlock(uint32_t aBlockNumber, Block &aBlock) {
    StatusResult result;
    if (cache->contains(aBlockNumber)) {
      aBlock = cache->get(aBlockNumber);
    }
    else {
      result = BlockIO::readBlock(aBlockNumber, aBlock);
  ...
  StatusResult  CachedBlockIO::writeBlock(uint32_t aBlockNumber, Block &aBlock) {
    cache->put(aBlockNumber, aBlock);
    return BlockIO::writeBlock(aBlockNumber, aBlock);
  }
  ...
  /*StatusResult Storage::markUsed(const char aChar, uint32_t aBlockNum) {
    //so probably don't want to use block header...
    return StatusResult{noError};
  }*/
  ...
  StatusResult Storage::releaseBlock(int aBlockNumber) {
    ...
    theBlock.header.next = 0; //not sure if needed
