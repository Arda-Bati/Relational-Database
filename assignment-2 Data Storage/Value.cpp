...
namespace ECE141 {
  ...
  Value& Value::operator=(uint32_t aValue) {
    timestamp = aValue;
    valueType = DataType::timestamp_type;
    return *this;
  }
  ...
  BufferWriter& operator << (BufferWriter& aWriter, const Value &aValue) {
    aWriter << static_cast<char>(aValue.valueType); //store the type
    switch(aValue.valueType) {
      case DataType::int_type: aWriter << aValue.intValue; break; 
      case DataType::float_type: aWriter << aValue.fltValue; break;
      case DataType::varchar_type: aWriter << aValue.varchar; break;
      case DataType::bool_type: aWriter << aValue.boolValue; break;
      case DataType::timestamp_type: aWriter << aValue.timestamp; break;
      case DataType::no_type: aWriter << false; break; //may change this later...
      default: break;
    }
    return aWriter;
  }
  ...
  BufferReader& operator >> (BufferReader& aReader, Value &aValue) {
    ...
    switch(theTypeChar) {
      case 'I':  { //int
       ...
       break;
       case 'T':  { //timestamp
        uint32_t theTimestamp{0};
        aReader >> theTimestamp; 
        aValue= theTimestamp; 
       }
       break;
       case 'V':  { //varchar
       ...
       break;
       case 'N':  { //no_type
        aValue=false; //Not sure about this...
       }
       break;
      default: break;
    }
  ...
  StatusResult  Value::become(DataType aType) {
    ...
    switch (valueType) {
      ...
      case ECE141::DataType::int_type       : try {intValue = std::stoi(varchar);} //do we always want a negative int value??
                                              catch (const std::exception& e) {intValue = -1;} break;
